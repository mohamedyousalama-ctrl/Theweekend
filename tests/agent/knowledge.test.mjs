...
test('the branch pack carries the owner decisions of 2026-09-14', () => {
  ...
  assert.match(byId.get('kno_mrs_booking_url').text_ar, /والتأكيد يصير من صفحة الحجز الرسمية\./);
  assert.match(byId.get('kno_mrs_booking_url').text_en, /confirmation comes from the official booking page\./);
  assert.doesNotMatch(byId.get('kno_mrs_booking_url').text_ar, /بعد الدفع|الدفع وحده/);
  assert.doesNotMatch(byId.get('kno_mrs_booking_url').text_en, /after payment|payment alone/i);
...
test('post-merge review: product availability, treatment claims, annual expiry and citable arithmetic', () => {
  const byId = new Map(pack.records.map((r) => [r.knowledge_id, r]));
  assert.equal(pack.pack_revision, 5);
