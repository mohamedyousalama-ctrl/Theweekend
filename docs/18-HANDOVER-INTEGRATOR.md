...
| Item | State |
|---|---|
| `main` = `7ab5a9d` (2026-09-19) | Since `3fb3d74` (PRs #35, #37–#45 — walkthrough runner, `/try`, public-guest opt-in, B5/B6 copy, C4/C5, try-page Send, C6, C7 — listed in the previous snapshot): **#46** (docs pins and state, 01:46 UTC), **#47** (C8 `/health` build and switch state, 02:13 UTC), **#48** (A4 eval `--staff-inbox`, 02:20 UTC), **#49** (B7 try-page polish, 02:54 UTC) and **#50** (C9 session-row retention, 06:20 UTC), all merged by the integrator under D17 after the two-agent review. Full SHA in `CONTINUE.md` §5. |
...
| Package C9 (PR #50) | **merged**. `sessions` rows and their dependents (`turns`, `allowed_actions`, `action_results`, `staff_handoffs`, `pending_requests`, plus `photo_observations` / `images` as a backstop) are deleted 30 days after `expires_at` (`SESSION_RETENTION_MS`, no env var), in one transaction, always after the 24 h client-key nulling. Subject-scoped tables (`subjects`, `preferences`, `permission_receipts`, `briefs`, `delivery_receipts`) are never touched by this sweep — kept independently on their own retention, or meant to outlive the session. `usage_records` is untouched too, but it is not subject-scoped: it is keyed on `session_id` only, has no cleanup anywhere in the codebase, and a deleted session leaves its usage/cost rows behind with a dangling `session_id`. |
| Bot reviews | Codex reviewed every PR marked ready through several rounds until its usage limit (2026-09-18 21:26 UTC); PR #43 `769a223` and PR #45 `7023618`–`0cd4065` were gated by the two-agent review only (recorded in both merge commits). Codex was back for #46 and #47 (no issues on the final heads) and the first head of #48, then hit its limit again (2026-09-19 02:19 UTC for #48 `471b33d`, 02:44 UTC for #49 and #50), so those heads were gated by the two-agent review only, recorded in each merge commit. CodeRabbit does not auto-review this repository (fewer than 10 stars). Cursor Bugbot: a comment whose text after `@cursor` starts with «review» triggers Bugbot and starts no Cursor agent — use `@cursor Apply …` (CONTINUE.md rules). |
...
2. **#8 owner walkthrough** with Khalid (domain from Railway → Settings → Networking, shared privately). Partial agent record: `docs/20`. Text eval is recorded (`docs/19`); vision eval still needs permitted faces. Owner-reported 2026-09-18 15:17 UTC: live `/customer.js` 200. Before the phone pass, confirm Railway deployed the current pin from `CONTINUE.md` §5 (`GET /health`'s `build.commit` should read `7ab5a9d` today; if it is `null`, fall back to confirming `GET /copy.js` contains `session.guest_closed`), not an older build.
