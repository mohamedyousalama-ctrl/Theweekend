...
| Package C9 (PR #50) | **merged**. `sessions` rows and their dependents (`turns`, `allowed_actions`, `action_results`, `staff_handoffs`, `pending_requests`, plus `photo_observations` / `images` as a backstop) are deleted 30 days after `expires_at` (`SESSION_RETENTION_MS`, no env var), in one transaction, always after the 24 h client-key nulling. Subject-scoped tables (`subjects`, `preferences`, `permission_receipts`, `briefs`, `delivery_receipts`, `usage_records`) are never touched by this sweep. |
