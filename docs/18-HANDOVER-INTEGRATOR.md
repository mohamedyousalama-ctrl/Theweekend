...
## 2. State snapshot (2026-09-19) — verify against #2 before trusting

| Item | State |
|---|---|
| `main` = `a985f58` (2026-09-19) | Since `3fb3d74` (PRs #35, #37–#45 — walkthrough runner, `/try`, public-guest opt-in, B5/B6 copy, C4/C5, try-page Send, C6, C7 — listed in the previous snapshot): **#46** (docs pins and state, 01:46 UTC), **#47** (C8 `/health` build and switch state, 02:13 UTC), **#48** (A4 eval `--staff-inbox`, 02:20 UTC), **#49** (B7 try-page polish, 02:54 UTC), **#50** (C9 session-row retention, 06:20 UTC), **#51** (docs pins and rows, 06:32 UTC) and **#52** (C10 usage-row retention, 06:54 UTC), all merged by the integrator under D17 after the two-agent review. Full SHA in `CONTINUE.md` §5. |
...
| Packages C4 + C7 (PRs #40, #45) | **merged**. Per-address public-guest limits (5 session creates / 10 min, 6 paid turns / min, 3 uploads / UTC day), an owner-reserved slice of the daily cap that guest spend cannot enter, settled guest spend tracked apart from in-flight reservations with the shared ledger capped at the guest share, `guest_settled_minor` backfilled on migrate, turn-limiter slot released when a reservation fails, empty-passcode `/try` probes no longer counted as passcode failures nor run through scrypt. Kill switch: an explicit `WEEKEND_PUBLIC_GUEST=false` wins, is read once at process start, and a deployment that starts with it off also ends guest sessions that were already open (`401 session.guest_closed`; `/try` shows the passcode gate with «انتهت جلسة التجربة»). |
| Package C5 (PR #41) | **merged**. Walkthrough runner fixes for #8. |
| Package B6 (PR #42) | **merged**. Booking copy (payment is not confirmation stays in the prompt and UI copy; the knowledge record keeps owner-supported wording only), try-page viewport, prompt `rakan.system.v0.9`. |
| Package C6 (PR #43) | **merged**. `staff_inbox` follows the store for every role (contract line unchanged); `talk_to_staff` / `share_brief_text` / `share_photo_ref` are refused while the inbox store is down — at issue (`defaultActions`, adapter proposals, share-actions `403 CAPABILITY_UNAVAILABLE`) and at execution, with the availability check before any consent ask; customer copy `staff_inbox.unavailable`. |
| Package C8 (PR #47) | **merged**. `GET /health` reports `public_guest` (the kill-switch state the process started with) and `build` (`started_at`; `commit` from `RAILWAY_GIT_COMMIT_SHA`, `null` when it is not set); `health-state.schema.json` carries both as optional fields. Still unauthenticated, nothing else exposed. |
| Package A4 (PR #48) | **merged**. `eval` takes `--staff-inbox enabled` or `--staff-inbox unavailable` (space or equals form; default `enabled`; any other value exits with a usage error). A case marked `requires_staff_inbox` is skipped before the paid adapter call when the flag does not match, and the report and console summary record `staff_inbox` and the skips. |
| Package B7 (PR #49) | **merged**. Try page: the passcode button reads «دخول» / «Enter», a caption under the field names the passcode source (the team), the shared passcode field is `autocomplete="off"`, and when the one automatic resend fails without a passcode gate the banner shows `try.recovery_failed` (reload or try again shortly) while keeping the real `code` / `retryable` / `details`. UI copy only, no contract change. |
| Package C9 (PR #50) | **merged**. `sessions` rows and their dependents (`turns`, `allowed_actions`, `action_results`, `staff_handoffs`, `pending_requests`, `usage_records` (since C10), plus `photo_observations` / `images` as a backstop) are deleted 30 days after `expires_at` (`SESSION_RETENTION_MS`, no env var), in one transaction, always after the 24 h client-key nulling. Subject-scoped tables (`subjects`, `preferences`, `permission_receipts`, `briefs`, `delivery_receipts`) are never touched by this sweep — kept independently on their own retention, or meant to outlive the session. |
| Package C10 (PR #52) | **merged**. `usage_records` — session-keyed, previously the sweep's only untouched dependent — is now deleted with its session too, in the same transaction, after `pending_requests` and before the photo backstop; no other change to the sweep. The retention test seeds a usage row for the old and the recent session and asserts gone/kept; docs/16, docs/17 and the CONTRACT list the table among the dependents. Found by the reviewer during the PR #51 docs review. |
| Bot reviews | Codex reviewed every PR marked ready through several rounds until its usage limit (2026-09-18 21:26 UTC); PR #43 `769a223` and PR #45 `7023618`–`0cd4065` were gated by the two-agent review only (recorded in both merge commits). Codex was back for #46 and #47 (no issues on the final heads) and the first head of #48, then hit its limit again (2026-09-19 02:19 UTC for #48 `471b33d`, 02:44 UTC for #49 and #50), so those heads were gated by the two-agent review only, recorded in each merge commit. Codex could not review #51 (06:23 UTC) or #52 (06:52 UTC) either. CodeRabbit does not auto-review this repository (fewer than 10 stars). Cursor Bugbot: a comment whose text after `@cursor` starts with «review» triggers Bugbot and starts no Cursor agent — use `@cursor Apply …` (CONTINUE.md rules). |
...
| #8 owner walkthrough | Railway live; `/health` 200. Partial record in `docs/20-OWNER-WALKTHROUGH-2026-09-18.md` (unauthenticated probes + static hash; authenticated turns skipped). Text eval in `docs/19`. Vision **NOT RUN**. Khalid phone walkthrough not done. The 2026-09-15/09-18 text evals ran with `staff_inbox: 'unavailable'` hardcoded in `eval.mjs`'s synthetic context, so they could not exercise a customer-facing `talk_to_staff` handoff; undiagnosed until C6. Fixed by A4 (`--staff-inbox`, default `enabled`); a re-run can now exercise the `talk_to_staff` case. |
| #10 Rekaz booking | READY-PENDING-CREDENTIALS (owner asks Khalid for the API key); after M1 |
| Hosting | Railway **project live for owner-review**. **Not released.** Passcodes live in Railway Variables. Kill switch for the passcode-free `/try`: set `WEEKEND_PUBLIC_GUEST=false` in Railway Variables, deploy that change and confirm the new deployment is live (`docs/17-HOSTING.md`); the value is read once at process start. After each merge, confirm the live build from `GET /health`: `build.commit` should match the pin in `CONTINUE.md` §5 and `public_guest` should match the intended switch state (falls back to `GET /copy.js` containing `session.guest_closed` while `build.commit` is `null`). |
| Open owner questions | MISSING-FACTS rows 18–19; photo share: observations only (D14) or the real photo later; confirm the visible name (D18 records «خالد»); keep `/try` passcode-free or close it; one shared team passcode; merge rule (keep D17 or «Cursor merges on my instruction, the integrator audits after»); regenerate the Railway domain (the old one is in Git history) and decide repository visibility; Rekaz key (#10); the #8 Khalid walkthrough. |
...
2. **#8 owner walkthrough** with Khalid (domain from Railway → Settings → Networking, shared privately). Partial agent record: `docs/20`. Text eval is recorded (`docs/19`); vision eval still needs permitted faces. Owner-reported 2026-09-18 15:17 UTC: live `/customer.js` 200. Before the phone pass, confirm Railway deployed the current pin from `CONTINUE.md` §5 (`GET /health`'s `build.commit` should read `a985f58` today; if it is `null`, fall back to confirming `GET /copy.js` contains `session.guest_closed`), not an older build.
...
6. **Kill switch** — decide whether `/try` stays passcode-free. To close it: set `WEEKEND_PUBLIC_GUEST=false` in Railway Variables, deploy that change and confirm the new deployment is live (`GET /health` shows `public_guest: false` with a new `build.commit`; `docs/17-HOSTING.md`); this also ends guest sessions that were already open. Keep `WEEKEND_TRUST_PROXY=1`.
7. **Decisions still open** — the visible name (D18 records «خالد»; confirm or choose Rakan); one shared team passcode (yes/no); and the merge rule: keep D17 (Cursor opens PRs, the integrator reviews and merges) or record a new decision that Cursor merges on your instruction and the integrator audits afterwards. B4 (**#34**), C3 (**#32**) and A3 (**#33**) reached `main` without a two-agent D17 review; #40 and #44 likewise (audited afterwards, fixes in #45).
