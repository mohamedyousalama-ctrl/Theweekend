...
- **Staff handoff (`talk_to_staff`)**: executing the action inserts a `received` row (not `accepted`). Model turns for that session pause until the row times out after **30 minutes** or is released. The application never claims staff acceptance on its own.
