...
| Current `main` after PRs #24–#52 | `a985f58044d0cc64600e2fb6c6d734c8b222f507` | verified (git), 2026-09-19 |
| `npm test` on `a985f58` | `npm test` → **470 pass, 0 fail, 0 skipped** (Node v22.22.2, npm 10.9.7, 2026-09-19 06:55 UTC) | tested, this checkout |
...
- **Public-guest session creates**: `WEEKEND_GUEST_SESSIONS_PER_10MIN` (optional, default 5) limits successful empty-passcode customer creates per client address in a rolling 10-minute window. Further creates from that client are `401 UNAUTHORIZED` (`session.throttled`, retryable), the same shape as passcode throttling. Authenticated customer, staff and owner creates are not counted. Passcode failures still use the existing attempt limiter. An empty-passcode customer create while public guest is off is refused (`session.passcode`) without counting against the passcode attempt limiter. Turning the switch off also ends guest sessions already open: once a process starts with `WEEKEND_PUBLIC_GUEST` off, every already-issued guest token is refused on its next call (`401`, `session.guest_closed`, not retryable) even before its 8-hour expiry, and still-live guest session rows are expired at start; a non-guest customer session (owner or local passcode) is unaffected.
- **Public-guest turns and reserved budget**: `WEEKEND_GUEST_TURNS_PER_MIN` (optional, default 6) limits paid public-guest turns per client address in a rolling minute (`429 BUDGET_EXCEEDED`, `session.throttled`, retryable). `WEEKEND_OWNER_RESERVED_USD_PER_DAY` (optional, default 20% of `WEEKEND_SPEND_CAP_USD_PER_DAY`, rounded down to a cent) is a slice of the daily cap that guest spend cannot enter. Owner and staff turns still run inside that reserve after guests have used their share. One log line (counts only) when guest spend first reaches 80% of its share and one at 100%. Guest vision/model calls count against that guest share, not a separate ledger. A guest turn's actual cost is clamped to the remaining guest share measured on settled guest spend (in-flight reservations do not count) when added to `guest_settled_minor`; any overage is logged (`ceiling_violation: true`) and further guest turns are refused for the rest of the UTC day. The shared cap ledger (`cost_minor`) only ever carries the guest side's contribution up to the share, so a guest actual that transiently pushes the raw guest ledger above the share while another guest turn is still in flight cannot consume the owner/staff reserve.
- **Public-guest uploads**: `WEEKEND_GUEST_UPLOADS_PER_DAY` (optional, default 3) limits photo uploads per client address per UTC day, counted from a durable per-day counter (`guest_upload_quota`, keyed by the client digest) that consent withdrawal does not reset. Further uploads are `400 UPLOAD_REJECTED` (`upload.rejected`). Authenticated customer, staff and owner uploads are not counted. A guest consent grant is not itself a vision call.
...
- **Session retention**: `sessions` rows (and their dependents in `turns`, `allowed_actions`, `action_results`, `staff_handoffs`, `pending_requests`, `usage_records` (no other retention anywhere in the codebase), plus `photo_observations`/`images` as a defensive backstop — normally already cleared by the 24 h photo-retention sweep) are deleted **30 days** after `expires_at` (`SESSION_RETENTION_MS`), always after the 24-hour client-key nulling. `subjects`, `preferences`, `permission_receipts` and `briefs` are never touched by this sweep: they are keyed on `subject_id`, not `session_id`, and either have their own retention (`preferences`, 90 days) or are meant to outlive the browser session (consent audit trail, approved briefs).
