...
- **Session retention**: `sessions` rows (and their dependents in `turns`, `allowed_actions`, `action_results`, `staff_handoffs`, `pending_requests`) are deleted **30 days** after `expires_at` (`SESSION_RETENTION_MS`), always after the 24-hour client-key nulling. `subjects`, `preferences`, `permission_receipts` and `briefs` are never touched by this sweep: they are keyed on `subject_id`, not `session_id`, and either have their own retention (`preferences`, 90 days) or are meant to outlive the browser session (consent audit trail, approved briefs).
