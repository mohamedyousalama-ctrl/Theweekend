...
| `WEEKEND_PUBLIC_GUEST` | optional; default `true` in owner-review (C4 guest limits apply), `false` in local. Set explicitly `false` in Railway Variables to close `/try` without a redeploy. |
