...
C2 (PR #24), the consent/brief UI (PR #25), then PRs #26–#52 (hub, guest limits and kill switch, staff inbox capability, `/health` build state, session-row and usage-row retention) are on `main` (`a985f58`, 2026-09-19). This is not a customer-facing release. Owner-reported 2026-09-18 15:17 UTC: live `GET /customer.js` HTTP 200 (CSP build). Confirm the Railway deployment SHA after each merge.
