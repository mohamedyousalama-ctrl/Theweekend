...
| Guest limiter counters | process memory only (`WindowCounter` / `AttemptLimiter`, keyed by the client digest, never the raw address) | rolling windows: 5 session creates / 10 min, 6 paid turns / min, 5 passcode failures / 15 min; cleared on restart |
| Guest sessions after the kill switch | SQLite `sessions` rows with `guest = 1` | expired at process start when `WEEKEND_PUBLIC_GUEST` is off; their tokens are refused as `session.guest_closed` |
