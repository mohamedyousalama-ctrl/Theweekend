...
| Deployed commit | auto-deploys `main`; confirm the live deployment SHA in the Railway dashboard after each merge (the current pin is in `CONTINUE.md` §5). |
| Health | **HTTP 200** at `<domain>/health` (rechecked 2026-09-19 01:18 UTC by the integrator; the build was still older than `3fb3d74` at that time). `HealthState` capabilities `ok`. |
