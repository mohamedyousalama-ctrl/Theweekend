...
| Session rows and dependents | SQLite `sessions`, `turns`, `allowed_actions`, `action_results`, `staff_handoffs`, `pending_requests` | deleted 30 days after `expires_at` (`SESSION_RETENTION_MS`), always after the 24 h client-key nulling. `subjects`, `preferences`, `permission_receipts` and `briefs` are not touched by this sweep — subject-scoped, kept independently (preferences on their own 90-day clock; consent receipts and approved briefs are meant to outlive the browser session). |
