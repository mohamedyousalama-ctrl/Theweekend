...
8. **Starting a Cursor agent** on an issue or PR: the comment begins `@cursor` followed by an instruction (`@cursor Apply …`). Never start the text after `@cursor` with the word «review»: that is Cursor Bugbot's keyword — it runs a bot review and starts no agent (learned 2026-09-18 on PR #43). Cursor cannot start a PR body or an issue on its own initiative; it reports in PR bodies and comments.
