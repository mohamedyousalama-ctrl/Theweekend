...
export function buildPack() {
  ...
  add({
    ...
    text_ar: `الحجز يتم من صفحة الحجز الرسمية: ${OFFICIAL_BOOKING_URL}?branchId=${STOREFRONT_BRANCH_ID} — يختار العميل الخدمة والحلاق والوقت بنفسه، والتأكيد يصير من صفحة الحجز الرسمية، والدفع وحده ما يأكّد الحجز. خالد ما يحجز ولا يغيّر الحجز بنفسه في هذه المرحلة.`,
    text_en: `Booking happens on the official page ${OFFICIAL_BOOKING_URL}?branchId=${STOREFRONT_BRANCH_ID}; the customer picks service, barber and time; confirmation comes only from the official booking page; payment alone does not confirm the booking. Khalid does not create or change bookings in this phase.`,
    source: OWNER.source,
    source_hash: OWNER.hash,
    status: 'merchant_approved',
    enabled: true,
